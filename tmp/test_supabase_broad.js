...
async function testFetch() {
    const tables = ["matches", "channels", "sports", "slideshow", "app_settings", "epg_schedules", "notifications"];
    for (const table of tables) {
        try {
            ...
            if (response.data.length > 0) {
                console.log(`First item sample from ${table}:`, JSON.stringify(response.data[0]).substring(0, 200));
            }
        } catch (e) {
